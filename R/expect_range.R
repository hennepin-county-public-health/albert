...
expect_range <- function(data, value_pairs, na_accept = FALSE, warn = FALSE){
  ...
  temp <- purrr::map(seq_along(value_pairs), function(x){
    ...
    if (is.na(temp_max) | is.na(temp_min)){
      stop(stringr::str_c(temp_col, ": Missing minimum or maximum value. Check that supplied format was numeric or YMD."))
    }

    #Stop if max is not greater than min
    if (temp_max <= temp_min){
      stop(stringr::str_c(temp_col, ": Maximum value must be greater than minimum."))
    }

    meta_col <- data |>
      dplyr::mutate("exp_{temp_col}_xrange" := dplyr::if_else(!!sym(temp_col) >= temp_min & !!sym(temp_col) <= temp_max, TRUE, FALSE), .keep = "none") |>
      #convert NA to TRUE or FALSE depending on argument
      dplyr::mutate(dplyr::across(tidyselect::everything(), ~ dplyr::if_else(is.na(.), as.logical(na_accept), .)))

    #If warn is true, print out number of violations
    if (warn == TRUE){
      viols <- nrow(dplyr::filter(meta_col, !!sym(names(meta_col)[1]) == FALSE))
      message(stringr::str_c(viols, " ", dplyr::if_else(viols == 1, "violation", "violations"), " of the expected *RANGE* in *", temp_col, "*"))
    }
