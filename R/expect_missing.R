...
expect_missing <- function(data, value_pairs, warn = FALSE){
  ...
  temp <- map(seq_along(value_pairs), function(c){
    ...
    #Get # of missing rows
    n_missing <- data |>
      dplyr::select(!!sym(temp_col)) |>
      dplyr::filter(is.na(!!sym(temp_col))) |>
      nrow()

    #Calculate % missing
    p_missing <- n_missing/nrow(data)
    log_value <- dplyr::if_else(p_missing > temp_perc, FALSE, TRUE)
    ...
    exp_col <- tibble::tibble(!!sym(cname) := rep(log_value, nrow(data)))
    return(exp_col)
