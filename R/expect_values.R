...
expect_values <- function(data, value_pairs, na_accept = FALSE, warn = FALSE){
  ...
  temp <- purrr::map(seq_along(value_pairs), function(x){
    ...
    meta_col <- data |>
      mutate("exp_{temp_col}_xvalue" := dplyr::if_else(!!sym(temp_col) %in% {temp_vals}, TRUE, FALSE), .keep = "none")

    #If warn is true, print out number of violations
    if (warn == TRUE){
      viols <- nrow(dplyr::filter(meta_col, !!sym(names(meta_col)[1]) == FALSE))
      message(stringr::str_c(viols, " ", dplyr::if_else(viols == 1, "violation", "violations"), " of the expected *VALUES* in *", temp_col, "*"))
    }
