...
expect_format <- function(data, value_pairs, na_accept = FALSE, warn = FALSE){
  ...
  temp <- purrr::map(seq_along(value_pairs), function(x){
    ...
    meta_col <- data |>
      dplyr::mutate("exp_{temp_col}_xformat" := dplyr::if_else(stringr::str_detect(!!sym(temp_col), temp_patrn), TRUE, FALSE), .keep = "none") |>
    ...
    if (warn == TRUE){
      ...
      message(str_c(viols, " ", dplyr::if_else(viols == 1, "violation", "violations"), " in the expected *FORMAT* of *", temp_col, "*"))
    }
