...
expect_type <- function(data, value_pairs, warn = FALSE){
  ...
  temp <- purrr::map(seq_along(value_pairs), function(t){

    temp_type <- stringr::str_to_lower(names(value_pairs)[t])
    temp_type <- dplyr::if_else(temp_type == "date", "Date", temp_type)
    ...
    meta_col <- purrr::map(seq_along(value_pairs[[t]]), function(c){
      ...
      type_test <- eval(parse(text = paste0(dplyr::if_else(temp_type == "Date", "lubridate::is.", "is."),
                                            temp_type, "(", dname, "$", temp_col, ")")))
      cname <- paste0("exp_", temp_col, "_xtype") #define exp_column name
      log_value <- tibble::tibble(!!sym(cname) := rep(type_test, nrow(data))) #create named tibble (returned as part of a list)

      #If warn is TRUE, notify of violations
      if (warn == TRUE & log_value[1,1] == FALSE){
        message(paste0("Expectation violated: *", temp_col, "* is not *", stringr::str_to_upper(temp_type), "* type"))
      }
  ...
  return(dplyr::bind_cols(data, bind_cols(temp)))
}
