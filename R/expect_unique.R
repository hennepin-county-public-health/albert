...
expect_unique <- function(data, columns, warn = FALSE){
  ...
  temp <- purrr::map(seq_along(columns), function(x){

    temp_col <- columns[[x]]
    temp_data <- data |>
      dplyr::select(tidyselect::all_of(temp_col))
    ...
    if (length(temp_col) == 2){
      temp_data <- temp_data |>
        dplyr::mutate(multi_col = paste(!!sym(temp_col[1]), !!sym(temp_col[2])), .keep = "unused")
    }

    if (length(temp_col) == 3){
      temp_data <- temp_data |>
        dplyr::mutate(multi_col = paste(!!sym(temp_col[1]), !!sym(temp_col[2]), !!sym(temp_col[3])), .keep = "unused")
    }

    if (length(temp_col) == 4){
      temp_data <- temp_data |>
        dplyr::mutate(multi_col = paste(!!sym(temp_col[1]), !!sym(temp_col[2]), !!sym(temp_col[3]), !!sym(temp_col[4])), .keep = "unused")
    }
    ...
    #Flag duplicates
    dupe_col <- (duplicated(dplyr::pull(temp_data[1]), incomparables = NA) | duplicated(dplyr::pull(temp_data[1]), fromLast = TRUE, incomparables = NA)) |>
      tibble::as_tibble() |>
      dplyr::mutate(value = !value) |>
      dplyr::rename(!!sym(cname) := value)

    #If warn is true, print number of violations
    if (warn == TRUE){

      cnt <- dupe_col |> dplyr::pull(1)
      cnt <- length(cnt[cnt == FALSE])
      message(stringr::str_c(cnt, " *DUPLICATE* ", dplyr::if_else(cnt == 1, "value", "values"),  " found in *", paste(temp_col, collapse = ","), "*"))
    }
    return(dupe_col)
  })
  return(dplyr::bind_cols(data, dplyr::bind_cols(temp)))
}
