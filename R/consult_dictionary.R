...
consult_dictionary <- function(dictionary, column_names, column_types, date_format = "%m/%d/%Y", datetime_format = "%m/%d/%Y %I:%M:%S %p"){
  ...
  dict <- dictionary |>
    dplyr::select(variable = tidyselect::all_of(column_names), type = tidyselect::all_of(column_types)) |>
  ...
  dlen <- nrow(dplyr::filter(dict, type == "date"))
  dtlen <- nrow(dplyr::filter(dict, type == "datetime"))
  dflen <- length(date_format)
  dftlen <- length(datetime_format)

  if (dflen != 1 & dflen != dlen){
    stop(stringr::str_c("date_format length should either be 1 or ", dlen, ", not ", dflen))
  }

  if (dftlen != 1 & dftlen != dtlen){
    stop(stringr::str_c("datetime_format length should either be 1 or ", dtlen, ", not ", dftlen))
  }
  ...
  for (x in 1:nrow(dict)){
    ...
    temp <- dict |> dplyr::slice(x)

    #there's got to be a better way.. but ifelse/casewhen don't work for this
    if (temp$type == "character"){
      col_types$cols[[temp$variable]] <- readr::col_character()
    } else if (temp$type == "numeric"){
      col_types$cols[[temp$variable]] <- readr::col_number()
    } else if (temp$type == "logical"){
      col_types$cols[[temp$variable]] <- readr::col_logical()
    } else if (temp$type == "date" & length(date_format) == 1){
      col_types$cols[[temp$variable]] <- readr::col_date(format = date_format)
    } else if (temp$type == "date" & length(date_format > 1)){
      date_count <- date_count + 1
      col_types$cols[[temp$variable]] <- readr::col_date(format = date_format[date_count])
    } else if (temp$type == "datetime" & length(datetime_format) == 1){
      col_types$cols[[temp$variable]] <- readr::col_datetime(format = datetime_format)
    } else if (temp$type == "datetime" & length(datetime_format) > 1){
      datetime_count <- datetime_count + 1
      col_types$cols[[temp$variable]] <- readr::col_datetime(format = datetime_format[datetime_count])
    } else {
      stop(stringr::str_c('Column type "', temp$type, '" not accepted. Accepted types are: character, date, datetime, logical, and numeric.'))
    }
